...
async function selectDate(req, res) {
  try {
    ...
    if (!dateString) return res.status(400).json({ error: "Missing date" });
...
async function createBooking(req, res) {
  try {
    const services = req.cookies.services
      ? JSON.parse(req.cookies.services)
      : [];
    if (!Array.isArray(services) || services.length == 0)
      return res
        .status(400)
    ...
    const date_taken = Booking.findOne({ date: date });
...
async function listAllBookings(req, res) {
  try {
    const bookings = await Booking.find({}) // fetch all bookings
      .populate("userID") // optional: include user info if userID references User
      .populate("services"); // optional: include service details too
